...
def get_hash_with_potential(atoms):
    """Creates a unique signature for a particular ASE atoms object 
       calculated within grand-canonical framework SJM.

    This is used to check whether an image has been seen before. Two
    identical images are defined as identical both in geometry and 
    electrode potential. This is just an md5 hash of a string 
    representation of the atoms object and target electrode potential.
    ...
    for number in atoms.get_positions().flatten():
        string += '%.15f' % number
    try: #images with SJM calculator
        number = atoms.calc.results['electrode_potential']
        string += '%.15f' % number
    except: #images with trained ML calculator
        number = atoms.calc.parameters['electrode_potential']
        string += '%.15f' % number
...
def hash_with_potential(images, log=None, ordered=False):
    """ Converts input images calculated within grand-canonical 
    framework SJM including geometry and electrode potential info
    -- which may be a list, a trajectory file, or
    a database -- into a dictionary indexed by their hashes.
    ...
    if images is None:
        ...
        # Need to be hashed, and possibly read from file.
        if isinstance(images, str):
            log('hash_with_potential called')
            log('Attempting to read images from file %s.' %
                images)
            extension = os.path.splitext(images)[1]
            from ase import io
        ...
        if ordered is True:
            ...
            dict_images = OrderedDict()
        for image in images:
            hash = get_hash_with_potential(image)
            if hash in dict_images.keys():
                log('Warning: Duplicate image (based on identical hash).'
                    ' Was this expected? Hash: %s' % hash)
                if hash in dup.keys():
                    ...
                    dup[hash] = 2
            dict_images[hash] = image
        log(' %i unique images after hashing with electrode potential info.' % len(dict_images))
        log('...hashing with electrode potential info completed.', toc='hash')
        return dict_images


def check_images(images, forces, charges):
    """Checks that all images have energies, and optionally forces
       and charges, calculated, so that they can be used for training. 
       Raises a MissingDataError if any are missing."""
    missing_energies, missing_forces, missing_charges = 0, 0, 0
    for index, image in enumerate(images.values()):
        ...
        if forces is True:
            try:
                ...
                missing_forces += 1
        if charges is True: #used when training charges
            try:
                try:
                    image.calc.parameters.sj['excess_electrons']
                except:
                    image.calc.results['ne']
            except PropertyNotImplementedError:
                missing_charges += 1
    if missing_energies + missing_forces + missing_charges == 0:
        return
    msg = ''
    ...
    if missing_charges > 0:
        msg += ' Missing charges in {} image(s).'.format(missing_charges)
...
def get_overfit_mask(nn_model, parametervector):
    ...
    if p['importname'] == '.model.chargeneuralnetwork.ChargeNeuralNetwork':
        charge_nn_weights = p.charge_weights
        charge_scalings = p.charge_scalings
   
        
        for key1 in elements:
            for key2 in sorted(charge_nn_weights[key1].keys()):
                ori_count = count
                lshape = np.shape(charge_nn_weights[key1][key2])
                count += int(lshape[0] * lshape[1])
                if key2 == 1:
                    overfit_mask[ori_count:(count - lshape[1])] = 1
        ...
        for key1 in elements:
            # overfit_mask[count:(count + 2)] = 0
            for key2 in sorted(charge_scalings[key1].keys()):
                count += 1
    ...
    # of parameter vector.
    msg = 'Counted parameters %d not equal to parametervector %d' % \
           (count, len(parametervector))
